            ...
            if ( !stopBuilding ) {
                ...
                if ( item.children ) {
                    ...
                    function buildOxygenTreeChild(counter) {
                        ...
                        // don't render child while parent is waiting for AJAX response
                        if ( parentComponent.hasClass('oxy-ajax-loading') && counter < 50) {
                            // timeout to wait for AJAX response
                            buildOxygenTreeChildTimeout(counter);
                        }
                        else {
                            // proceed with rendering elements
                            $scope.buildingOxygenTreeCounter--;
            ...
            switch (componentName) {
                ...
                default:
                    ...
                    if ($scope.componentsTemplates !== undefined && $scope.componentsTemplates[componentName]) {
                        ...
                        if (typeof($scope.component.options[id]['model']['renderedHTML']) === 'undefined' && componentTemplate.phpCallback) {
                            // render with AJAX
                            var timeout = $timeout(function() {
                    ...
                    classes = 'class="oxy-ajax-loading '+componentClass+' {{getComponentsClasses('+id+', \''+componentName+'\')}}" ';
    ...
    $scope.rebuildDOM = function(id, reorder, domNode) {
        ...
        if ( $scope.componentsTree.children ) {
            ...
            function waitOxygenTreeRebuild(counter) {
                if ( $scope.buildingOxygenTreeCounter > 0 && counter < 50) {
                    // keep timeout
