...
CTFrontendBuilder.controller("ControllerCSS", function($scope, $parentScope, $http, $timeout) {
		...
		for(var stateName in componentOptions) { 
			if (componentOptions.hasOwnProperty(stateName)) {
				...
				if ($scope.componentsTemplates !== undefined && 
		        	...
		        	$scope.globalWooSettingsAdded[componentOptions.name] !== true ) {
				...
				if ($scope.componentsTemplates !== undefined && 
		        	...
		        	var selectors = $scope.componentsTemplates[componentOptions.name]['registeredSelectors'],
		        		complexProperties = angular.copy($scope.componentsTemplates[componentOptions.name]['complexProperties']),
		        		selectorsCSS = "";
		        	...
		        	for(var key in selectors) { 
						if (selectors.hasOwnProperty(key)) {
								...
								for(var property in selector) { 
									if (selector.hasOwnProperty(property)) {
										...
										// plain options
											...
											selectorCSS += property+":"+$scope.getGlobalColorValue(value)+";";
										}
											...
											for(var option in stateOptions) { 
												if (stateOptions.hasOwnProperty(option)) {
													if (option.indexOf(selector[property])>-1) {
														...
														// load Web Fonts
														selectorCSS += unprefixedOption+":"+$scope.getGlobalColorValue(value)+";";
													}
												}
											}
										}
							...
							if (selectorCSS) {
								...
								selectorsCSS += $scope.prefixCSSSelectors(selectorCSS,componentOptions.selector);
							}
						}
					}
    ...
    $scope.prefixCSSSelectors = function(css, selector) {
		...
		for (var key in parts) {
			if (parts.hasOwnProperty(key)) {
				...
				for (var subPartKey in subParts) {
					subParts[subPartKey] = '#' + selector + ' ' + subParts[subPartKey];
				}
