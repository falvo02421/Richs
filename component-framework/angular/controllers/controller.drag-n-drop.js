    ...
    $scope.$parent.$on('iframe-init', function(e, iframeScope) {
        ...
        dragulaService.options($scope.iframeScope, 'ct-dom-tree', {
            ...
            accepts: function (el, target, source, sibling) {
                ...
                // don't allow to insert 'if and else' to any components but 'if_else_wrap'
                if( (el.classList.contains('ct-sortable-if-wrap') || el.classList.contains('ct-sortable-else-wrap')) && !target.classList.contains('ct-sortable-if-else-wrap') ) {
                    return false;
                }

                // don't allow to insert 'anything excep if and else' under 'if_else_wrap'
                if( (!el.classList.contains('ct-sortable-if-wrap') && !el.classList.contains('ct-sortable-else-wrap')) && target.classList.contains('ct-sortable-if-else-wrap') ) {
                    return false;
                }
