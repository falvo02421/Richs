...
CTFrontendBuilder.controller("ControllerAJAX", function($scope, $parentScope, $http, $timeout) {
    ...
    /**
     * Load components templates HTML codes
     * 
     * @since 2.3
     * @author Ilya K.
     */
...
//                            console.log($scope.buildingOxygenTreeCounter);

                            // don't render child while parent is waiting for AJAX response
                            if ( $scope.buildingOxygenTreeCounter > 0 && counter < 30) {
                                // keep timeout
    ...
    $scope.loadControlsWithAJAX = function(name) {
        ...
        .success(function(data, status, headers, config) {
            if(data){
                ...
                console.log("No controls for " + name);
            }
            $parentScope.showSidebarLoader = false;
        })
        .error(function(data, status, headers, config) {
