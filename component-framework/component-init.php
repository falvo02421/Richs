...
$media_queries_list_above = array (
	...
	"tablet" 	=> array (
					"minSize" 	=> '992px', 
					"title" 	=> "At or above 992px"
				),
	"phone-landscape" 
				=> array (
					"minSize" 	=> '768px', 
					"title" 	=> "At or above 768px"
				),
	"phone-portrait"
				=> array (
					"minSize" 	=> '480px', 
					"title" 	=> "At or above 480px"
				),
);
...
function oxy_register_condition_post_type() {
	...
	oxygen_vsb_register_condition('Archive Post Type', array('options'=> array_keys($postTypes)), $oxy_condition_operators['simple'], 'oxy_condition_post_type_callback', 'Archive');	
}
...
function oxy_condition_taxonomy_term_callback($value, $operator) {
	...
	$is_tax = is_category($value) || is_tag($value);

	if(term_exists($value)) {
		$term = get_term( $value );
		if($term && $term->taxonomy) {
			$is_tax = $is_tax || is_tax($term->taxonomy, $value);
		}
	}
