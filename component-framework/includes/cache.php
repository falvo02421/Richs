...
function oxygen_vsb_element_inline_styles($options) {
	...
	if ( isset($options['background-image']) && !empty($options['background-image']) && strpos(($options['background-image']),'[oxygen') !== false ) {
		...
		$editable_background = 'url(' . do_shortcode($options['background-image']) .')';
	} else if( isset($options['background-image']) && !empty($options['background-image']) && $options['tag'] == 'section' ) {
		$styles = ct_getBackgroundLayersCSS($options);
		$editable_background = 'url(' . $options['background-image'] .')';
    }

	if($editable_background == 'url()') $editable_background = '';

	//var_dump($options);
	$in_gutenberg = !empty( $_GET['oxygen_gutenberg_script'] );
	...
	if( ( $in_gutenberg || $editable_background != 'auto' ) && isset($options['tag']) && $options['tag'] == 'section' ) {
