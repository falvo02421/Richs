...
function ct_view_meta_box_callback( $post ) {
	...
	if ($screen->post_type != "ct_template") {
		...
		?>
			...
			$parent_dropdown = ob_get_clean();
				...
				<div id="ct-edit-template-builder-parent-wrap" <?php echo (!$show_edit_button && $option_is_selected) ? "":"style='display:none' " ?>>
					<a data-site-url="<?php echo site_url();?>" data-parent-template="<?php echo $ct_other_template?intval($ct_other_template):0;?>" href="<?php echo esc_url(ct_get_post_builder_link( $template_id )); echo $parent_of_template?'&ct_inner=true':''; ?>" id="ct-edit-template-builder-parent" class="button button-primary">
						<?php printf( __("Edit Template", "component-theme") ); ?>
					</a>
					<p>This post is being rendered by an Oxygen template. To edit this post directly, add an Inner Content element to the template.</p>
				</div>

				
	
				<a <?php echo (!$show_edit_button && $option_is_selected) ? "style='display:none' ":"" ?>data-parent-template="<?php echo $ct_other_template?intval($ct_other_template):0;?>" href="<?php echo esc_url(ct_get_post_builder_link( $post->ID )); echo (($shortcodes && strpos($shortcodes, '[ct_inner_content') !== false) && intval($ct_other_template) !== -1)?'&ct_inner=true':''; ?>" id="ct-edit-template-builder" class="button button-primary">
		...
		<?php
        Oxygen_Revisions::render_revisions_list( $post->ID );
	}
	// Button only for "Views"
		...
		$ct_template_type = get_post_meta( $post->ID, 'ct_template_type', true);
			...
			<div class='oxygen-edit-template-button-or-message'>
				<a data-parent-template="<?php echo $ct_parent_template?intval($ct_parent_template):0;?>" href="<?php echo esc_url(ct_get_post_builder_link( $post->ID )); echo ($shortcodes && strpos($shortcodes, '[ct_inner_content') !== false)?'&ct_inner=true':''; ?>" id="ct-edit-template-builder" class="button button-primary">
