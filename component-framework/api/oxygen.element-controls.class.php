...
//			$this->add_error("<br/><b>Warning:</b> no contorls found for controlsReady()");
	...
	function output_controls() {
		...
		$this->component->component_params($this->get_controls(), $this->get_tag());
		echo '</div>';

		// controls sections
		if (!empty($this->params['tabs'])) {
			echo $this->component->component_tabs($this->get_section_controls(), $this->get_tag(), $this->params['title']);
		}
	}
