...
class Oxygen_VSB_Dynamic_List extends CT_Component {
    ...
    function resolve_recursively($arr) {
        ...
        if(is_array($arr)) {
            foreach($arr as $key => $value) {
                ...
                elseif($key === 'globalconditions') {
                    if($value && sizeof($value) > 0) {
                        $results['globalConditionsResult'] = oxy_vsb_globalConditionsResult(array('conditions'=>$value, 'type'=>isset($arr['conditionstype'])?$arr['conditionstype']:''));
                    }
                }
                elseif(is_array($value)) {
                    $result = $this->resolve_recursively($value);
                } elseif($key === 'renderedHTML') {
                    $result = '';
                    ...
                    if(is_array($oxy_el_slug_classes) && isset($oxy_el_slug_classes[$slug])) {
                        ...
                        ob_start();
                            $element_instance->render($el->unprefix_options($options), $el->unprefix_options($el->defaults), $content);
                        $result = ob_get_clean();    
                    }                    

                } 
                elseif($key !== 'globalConditionsResult' && $key !== 'component') { // this has already been taken care of
                    $result = do_shortcode($value);
                }

                
                if($key !== 'globalConditionsResult' && $key !== 'component') { // because this value is expicitly set above or not required
                    $results[$key] = $result;
                }
            }
        }
    ...
    function parse_shortcodes_map_old($models, $options) {
