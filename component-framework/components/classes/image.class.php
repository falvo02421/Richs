...
Class CT_Image extends CT_Component {
	...
	function add_shortcode( $atts, $content, $name ) {
		...
		if( $options['image_type'] == 1 ) {
			$editable_attribute = $options['src'];
			if( class_exists( 'Oxygen_Gutenberg' ) ) $editable_attribute = Oxygen_Gutenberg::decorate_attribute( $options, $editable_attribute, 'image' );
			echo '<img id="' . esc_attr( $options['selector'] ) . '" alt="' . esc_attr( $options['alt'] ) . '" src="' . $editable_attribute . '" class="' . esc_attr( $options['classes'] ) . '"'; do_action("oxygen_vsb_component_attr", $options, $this->options['tag']); echo '/>';
		} else {
			$source = wp_get_attachment_image_src( intval( $options['attachment_id'] ), (isset($options['attachment_size']) ? $options['attachment_size'] : 'thumbnail') );

			if( is_array( $source ) ) {
				$src = $source[0];
				$image_width = $source[1];
			} else {
				$src = "";
				$image_width = "";
			}

			$editable_attribute = $src;
			if( class_exists( 'Oxygen_Gutenberg' ) ) $editable_attribute = Oxygen_Gutenberg::decorate_attribute( $options, $editable_attribute, 'image' );
			...
			$srcset = wp_get_attachment_image_srcset( intval( $options['attachment_id'] ), (isset($options['attachment_size']) ? $options['attachment_size'] : 'thumbnail') );
			...
			echo '<img id="' . esc_attr( $options['selector'] ) . '" alt="' . esc_attr( $options['alt'] ) . '" src="' . esc_attr($src) . '" class="' . esc_attr( $options['classes'] ) . '" srcset="' . $srcset . '" sizes="(max-width: '.$image_width.'px) 100vw, '.$image_width.'px" '; do_action("oxygen_vsb_component_attr", $options, $this->options['tag']); echo '/>';
		}
